...
import { addInvestments, editInvestments } from 'app/dashboard/investments/apis';
...
interface AdddInvestmentsProps {
	show: boolean;
	selected: any;
	onHide: () => void;
	mutate: () => void;
...
export default function AddInvestments({ show, onHide, mutate, selected, lookup }: AdddInvestmentsProps) {
	const user = useUser();
	...
	const onSubmit = async () => {
		try {
			...
			if (isEditing) {
				await editInvestments(state);
			} else {
				await addInvestments(state);
			}
			setLoading(false);
	...
	return (
		<Modal someRef={inputRef} show={show} title={`${selected.id ? 'Editar' : 'Adicionar'} Investimento`} onHide={onHide}>
